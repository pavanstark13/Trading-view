...
function CollapsiblePanel({ title, icon, defaultOpen = true, badge, children }: {
  title: string
  icon: string
  defaultOpen?: boolean
  badge?: ReactNode
  ...
  return (
    <div className="panel">
      <div className="panel-header" style={{ cursor:'pointer', userSelect:'none' }}
        ...
        <span>{icon}</span>
        <span>{title}</span>
        {badge && <span style={{ marginLeft:'auto', marginRight:6 }}>{badge}</span>}
        <span style={{ marginLeft: badge ? 0 : 'auto', fontSize:9, color:'#484f58',
...
export default function AIPanel({ analysis: a, loading, pair, account, riskPct, pipSize }: Props) {
  ...
  const bCol     = biasColor(a.bias)

  return (
    <div className="flex flex-col gap-2 fade-in" style={{ fontSize:11 }}>

      {/* Verdict */}
      <CollapsiblePanel title="AI Verdict" icon="🤖" defaultOpen
        badge={<span style={{ fontSize:11, fontWeight:900, color:bCol }}>{a.bias}</span>}>
        <div style={{ padding:'10px 12px', textAlign:'center' }}>
          <div style={{ fontSize:24, fontWeight:900, letterSpacing:'0.1em',
            color:bCol, textShadow:`0 0 20px ${bCol}50` }}>
            {a.bias==='BULLISH'?'▲ ':a.bias==='BEARISH'?'▼ ':'● '}{a.bias}
          </div>
          <div style={{ color:'#8b949e', fontSize:10, marginTop:4 }}>
            {pair} · {a.session}
            {a.bestSession
              ? <span style={{ color:'#00c853', marginLeft:6 }}>✔ Prime</span>
      ...
      {/* Scores */}
      <CollapsiblePanel title="Confluence" icon="📊" defaultOpen
        badge={<span style={{ fontSize:9, color:'#8b949e' }}>{a.longScore}L / {a.shortScore}S</span>}>
          ...
          {[
            ...
            <div key={label}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                ...
                <span style={{ color:scoreColor(score, a.maxScore), fontWeight:800 }}>{score}/{a.maxScore}</span>
              </div>
      ...
      {/* Signal log */}
      <CollapsiblePanel title="AI Thinking Log" icon="🧠" defaultOpen={false}>
        <div>
          ...
          {a.signals.map((sig, i) => (
            <div key={i} className="signal-row" style={{ background:i%2===0?'#161b22':'#1c2128' }}>
              ...
              <div style={{ display:'flex', gap:14 }}>
                <span style={{ color:sig.long ?'#00c853':'#30363d', width:28, textAlign:'center', fontWeight:800 }}>{sig.long ?'✔':'–'}</span>
                <span style={{ color:sig.short?'#ff1744':'#30363d', width:28, textAlign:'center', fontWeight:800 }}>{sig.short?'✔':'–'}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Trade idea */}
      {tradeDir && (
        <CollapsiblePanel title="Trade Idea" icon="💡" defaultOpen
          badge={<span style={{ color:tradeDir==='LONG'?'#00c853':'#ff1744', fontWeight:900, fontSize:10 }}>
          ...
          <div style={{ padding:'4px 0' }}>
            {[
              ...
              { label:'Lots',     value:'≈ '+lots.toFixed(2)+' lots',                                  c:'#8b949e' },
            ].map(({ label, value, c }, i) => (
      ...
      {/* Market context */}
      <CollapsiblePanel title="Market Context" icon="🌐" defaultOpen={false}>
        <div style={{ padding:'4px 0' }}>
          {[
...
// ─ Collapsible wrappers re-exported for use in other panels ─
export { CollapsiblePanel }
// Suppress TS unused warning
void useRef; void useCallback

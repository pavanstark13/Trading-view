...
const csLabel = (v: number) =>
  v >= 75 ? 'V.STR' : v >= 60 ? 'STR' : v <= 25 ? 'V.WK' : v <= 40 ? 'WK' : 'NEU'
...
export default function StrengthMeter({ strength }: Props) {
  ...
  const badge = (
    <span style={{ fontSize: 9, color: '#8b949e' }}>
      <span style={{ color: '#00c853' }}>{best}</span>
      {' vs '}
      <span style={{ color: '#ff1744' }}>{worst}</span>
    </span>
  )
  ...
  return (
    <CollapsiblePanel title="💱 Currency Strength" defaultOpen={false} badge={badge}>
      <div style={{ padding: '6px 12px 8px' }}>
        {sorted.map(([cur, val]) => (
          <div key={cur} style={{ marginBottom: 5 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              <span style={{ color: CUR_COLORS[cur] ?? '#fff', fontWeight: 700, fontSize: 11 }}>{cur}</span>
        ...
        <div style={{ marginTop: 6, fontSize: 9, color: '#8b949e' }}>
          Best: <span style={{ color: '#ffd600' }}>{best}{worst}</span>
          &nbsp;·&nbsp; Weakest: <span style={{ color: '#ff1744' }}>{worst}</span>
        </div>
      </div>
    </CollapsiblePanel>
  )

...
const PIP: Record<string,number> = { USDJPY:0.01, GBPJPY:0.01, EURJPY:0.01, CADJPY:0.01, AUDJPY:0.01 }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'#0d1117' }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', marginLeft:'auto' }}>
          <label style={{ fontSize:10, color:'#8b949e', display:'flex', alignItems:'center', gap:4 }}>
            ...
            <input type="number" value={account} onChange={e => setAccount(+e.target.value)}
              style={{ width:72, padding:'2px 5px', background:'#21262d', border:'1px solid #30363d',
          ...
          <label style={{ fontSize:10, color:'#8b949e', display:'flex', alignItems:'center', gap:4 }}>
            ...
            <input type="number" value={riskPct} onChange={e => setRiskPct(+e.target.value)}
              ...
              style={{ width:48, padding:'2px 5px', background:'#21262d', border:'1px solid #30363d',
                borderRadius:4, color:'#e6edf3', fontSize:10, fontFamily:'inherit' }} />
          </label>
        </div>
        ...
        {effectiveBias && (
          <div style={{ padding:'3px 10px', borderRadius:4, fontSize:11, fontWeight:700,
            ...
            {customSig.bullish + customSig.bearish > 0 &&
              <span style={{ fontSize:9, marginLeft:4, opacity:0.7 }}>+strat</span>}
          </div>
        )}
      ...
      {error && (
        <div style={{ background:'#3d0d14', color:'#ff1744', padding:'5px 12px',
          fontSize:11, borderBottom:'1px solid #ff174440' }}>⚠ {error}</div>
      )}
        ...
        <div style={{ flex:1, minWidth:0, border:'1px solid #30363d',
          ...
          {candles.length > 0
            ...
            : <div style={{ display:'flex', alignItems:'center', justifyContent:'center',
                ...
                {loading
                  ? <span className="animate-pulse">Loading {pair}…</span>
      ...
      <div style={{ padding:'3px 12px', background:'#161b22', borderTop:'1px solid #30363d',
        fontSize:9, color:'#444d56', display:'flex', justifyContent:'space-between', flexShrink:0 }}>
        <span>Forex AI · SMC + ICT + Strategy Feed · Backtest + Forward Test · Auto-refresh 60s</span>

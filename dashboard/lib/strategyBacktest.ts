...
function evalRule(
  ...
  switch (rule.indicator) {
    case 'rsi': {
      const v = ind.rsi[i]; if (isNaN(v)) return false
    ...
    case 'macd': return rule.direction === 'bull' ? ind.macdL[i] > ind.macdS[i] : ind.macdL[i] < ind.macdS[i]
    case 'ema': {
    ...
    case 'trend': return rule.direction === 'bull' ? ind.e20[i] > ind.e50[i] && ind.e50[i] > ind.e200[i] : ind.e20[i] < ind.e50[i] && ind.e50[i] < ind.e200[i]
    case 'stoch': {
      const k = ind.stochK[i], d = ind.stochD[i]
      if (rule.condition === 'oversold')   return k < 25
      if (rule.condition === 'overbought') return k > 75
      if (rule.condition === 'bull_cross') return i > 0 && k > d && ind.stochK[i-1] <= ind.stochD[i-1]
      return false
    }
    case 'vwap': return rule.direction === 'bull' ? c.close > ind.vwap[i] : c.close < ind.vwap[i]
    case 'smc': return rule.direction === 'bull' ? ind.e20[i] > ind.e50[i] : ind.e20[i] < ind.e50[i]
    case 'vol': return (c.volume ?? 0) > ind.volAvg[i] * 1.3
    case 'candle': {
...
export function runBacktest(candles: Candle[], text: string): BacktestResult {
  ...
  if (candles.length < 60)
    return { trades:0, winRate:0, profitFactor:0, maxDrawdown:0, avgWin:0, avgLoss:0,
      passed:false, failReason:'Need at least 60 candles to backtest.', rulesFound: rules.map(r => r.label) }

  const closes = candles.map(c => c.close), n = candles.length
  ...
  let cumPV = 0, cumV = 0
  const vwap = candles.map(c => { const tp=(c.high+c.low+c.close)/3, v=c.volume??1; cumPV+=tp*v; cumV+=v; return cumPV/cumV })
  ...
  const HOLD = Math.max(5, Math.min(15, Math.floor(n / 20)))
  ...
  for (let i = 30; i < n - HOLD - 1; i++) {
    ...
    if (bullRules.length > 0) {
      ...
      if (fired >= Math.ceil(bullRules.length * 0.5)) {
        const pnl = (candles[i + HOLD].close - candles[i + 1].open) / atrV
  ...
  if (total < 5)
    return { trades: total, winRate:0, profitFactor:0, maxDrawdown:0, avgWin:0, avgLoss:0,
      passed:false, failReason:`Only ${total} signals fired — strategy too selective or rules too strict.`,
  ...
  // Minimum 72% win rate required
  const passed = winRate >= 72 && profitFactor >= 1.5
  const failReason = passed ? undefined
    : winRate < 72
      ? `Win rate ${winRate.toFixed(1)}% is below the 72% minimum (${wins.length}W / ${losses.length}L)`
      : `Profit factor ${profitFactor.toFixed(2)} is below 1.5 minimum`

  return { trades: total, winRate, profitFactor, maxDrawdown: maxDD, avgWin, avgLoss, passed, failReason,
    rulesFound: rules.map(r => r.label) }
}

export function evalForwardSignal(candles: Candle[], text: string): 'long' | 'short' | null {
  if (candles.length < 30) return null
  const rules = extractRules(text)
  const closes = candles.map(c => c.close), n = candles.length - 1
  const rsiArr = rsi(closes, 14)
  const { macd: macdL, signal: macdS } = macd(closes)
  const e20 = ema(closes, 20), e50 = ema(closes, 50), e200 = ema(closes, 200)
  const { k: stochK, d: stochD } = stoch(candles)
  ...
  const bullRules = rules.filter(r => r.direction === 'bull')
  const bearRules = rules.filter(r => r.direction === 'bear')
  const bullFired = bullRules.length > 0 ? bullRules.filter(r => evalRule(r, n, candles, ind)).length / bullRules.length : 0
  const bearFired = bearRules.length > 0 ? bearRules.filter(r => evalRule(r, n, candles, ind)).length / bearRules.length : 0
